...
function sum(numArray) {
  let result = 0;
  let amount = numArray.reduce(function(a, b) {
    return a + b;
  }, 0);
  result += amount;
  return result;
...
function doubleNumbers(arrayOfNums) {
  let result = [];
  result = arrayOfNums.map(function(num) {
    return num * 2;
  });
  return result;
};
...
function multiplyNumbers(multiplicands, multiplier) {
  let result = [];
  result = multiplicands.map(function(num) {
...
function arraysToObject(array) {
  ...
  for (let i = 0; i < array.length; i++) {
    arrayArray = array[i];
    result[arrayArray[0]] = arrayArray[1];
  }
  return result;
};
...
function repeats(string) {
  ...
  for (let i = 0; i < string.length / 2; i++) {
    result = (string[i] === string[i + string.length / 2])
  }
  return result;
};
...
function letterCount(string) {
  let result = {};
  for (let i = 0; i < string.length; i++) {
    result[string[i]] = result[string[i]] || 0;
    result[string[i]]++;
  }
  return result;
};
...
function leftPad(string, length, fill) {
  ...
  if (string.length >= length) {
    ...
    while (result.length < difference) {
      result += fill;
    }
    result += string;
    return result;
...
function index(array, propName) {
  let result = {};
  for (let i = 0; i < array.length; i++) {
    result[array[i][propName]] = array[i];
  }
  return result;
};
...
function addSignature(name, object) {
  let result = {};
  for (let key in object) {
    let keySuff = key + "-signed";
    let valueSuff = object[key] + " - " + name;
    result[keySuff] = valueSuff;
  }
  return result;
};
